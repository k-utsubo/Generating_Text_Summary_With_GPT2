...
def write_json(i, article, abstract):
    ...
    file = "/content/CNN/gpt2_1024_data/"+str(i)+".json"
    js_example = {}
    js_example['id'] = i
...
def main(file_names, directory):
    ...
    for file in file_names:
        ...
        article, abstract = tokenizer.encode(
            article), tokenizer.encode(abstract)
...
if __name__ == '__main__':
    ...
    # only consider files with total no of tokens less than 1024
    file_names = [file for file, size in file_sizes.items() if size <= 1023]
