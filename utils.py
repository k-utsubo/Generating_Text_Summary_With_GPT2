        ...
        for _ in range(length-1):
            ...
            for j in range(len(sequences)):
                new_generated = torch.cat((context, torch.tensor(
                    [sequences[j]], dtype=torch.long, device=device)), dim=1)
                inputs = {'input_ids': new_generated}
                outputs = model(**inputs)
                next_token_logits = outputs[0][0, -1, :] / temperature
                next_token_probs = F.softmax(next_token_logits)
                start, stop = j*len(next_token_logits), (j+1) * \
...
def generate_beam_sample(model, data, tokenizer, num=1, length=100, beam_size=3, device=torch.device('cuda')):
    for i in range(num):
        ...
        scores, sequences = beam_search(
            model, context, length, beam_size, device)
        ...
        for i in range(len(sequences)):
            ...
            print("generated_summary-{} and Score is {}.".format(i +
                                                                 1, scores[i]), end='\n\n')
            print(text, end='\n\n')


def generate_sample(model, data, tokenizer, num=1, eval_step=False, length=100, temperature=1, top_k=10, top_p=0.5, device=torch.device('cuda')):
    for i in range(num):
        sample = data[i]
        idx = sample['sum_idx']
        context = sample['article'][:idx].tolist()
        ...
        generated_text = sample_seq(
            model, context, length, device, temperature, top_k, top_p)
